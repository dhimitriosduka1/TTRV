...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.reward_model.reward_manager == "ttrl":
            ...
            self.n_samples_per_prompt = (
                self.config.reward_model.reward_kwargs.n_samples_per_prompt
            )
            self.n_votes_per_prompt = (
                self.config.reward_model.reward_kwargs.n_votes_per_prompt
            )
        else:
            self.use_ttrl = False
    ...
    def _create_dataloader(self, seed=1):
        ...
        if (
            "custom_cls" in self.config.data
            and self.config.data.custom_cls.get("path", None) is not None
        ):
            dataset_cls = load_extern_type(
                self.config.data.custom_cls.path, self.config.data.custom_cls.name
            )
    ...
    def _select_top_k_per_prompt(self, data, n_votes_per_prompt, n_samples_per_prompt):
        assert (
            len(data) % n_votes_per_prompt == 0
        ), "data length must be divisible by n_votes_per_prompt"
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        if self.use_ttrl:
                            assert (
                                ...
                                == len(batch) * self.n_votes_per_prompt
                            )
                        else:
                            pass
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.use_ttrl:
                            sorted_indices = sorted(
                                range(len(batch)),
                                key=lambda i: batch[i].non_tensor_batch["extra_info"][
                                    "index"
                                ],
                            )
                        ...
                        try:
                            ...
                            if self.use_ttrl:
                                ...
                                post_reward_result = (
                                    self.reward_fn.compute_post_ttrl_metrics(batch)
                                )
