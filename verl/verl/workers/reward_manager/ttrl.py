...
class TTRLRewardManager:
    ...
    def __init__(
        ...
        print(
            f"TTRLRewardManager initialized with n_votes_per_prompt {n_votes_per_prompt}, n_samples_per_prompt {n_samples_per_prompt}, eval_n_samples {eval_n_samples}"
        )
    ...
    def _data_source_to_task(self, data_source):
        if data_source in ["MATH-TTT", "AIME-TTT", "AMC-TTT", "data/AIME-TTT"]:
            ...
            raise NotImplementedError(
                f"Data source {data_source} is not supported for TTRLRewardManager"
            )
    ...
    def compute_post_ttrl_metrics(self, data: DataProto):
        ...
        """
        assert (
            len(data) % self.n_samples_per_prompt == 0
        ), f"Length of data {len(data)} should be divisible by n_votes_per_prompt {self.n_samples_per_prompt}"
    ...
    def _compute_ttrl_reward(self, data: DataProto):
        ...
        assert (
            len(data) % self.n_votes_per_prompt == 0
        ...
        for prompt_i in range(prompt_num):
            ...
            for i in range(self.n_votes_per_prompt):
                if i < self.n_samples_per_prompt:
                    reward_tensor[
                        prompt_i * self.n_samples_per_prompt + i,
